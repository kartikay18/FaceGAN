...
def train_dcgan_labeled(gen, dis, epoch0=0):
    ...
    zvis = (np.random.uniform(-1, 1, (100, nz))).astype(np.dtype(np.float32))
    ...
    for epoch in xrange(epoch0,n_epoch):
        ...
        for i in xrange(0, n_train, batchsize):
            ...
            x2 = np.zeros((batchsize, 3, 96, 96)).astype(np.dtype(np.float32))
            for j in range(batchsize):
                try:
                    rnd = np.random.randint(len(dataset))
                    rnd2 = np.random.randint(2)

                    img = np.asarray(Image.open(StringIO(dataset[rnd])).convert('RGB')).transpose(2, 0, 1).astype(np.dtype(np.float32))
                    if rnd2==0:
                        x2[j,:,:,:] = (img[:,:,::-1]-128.0)/128.0
                    else:
                        x2[j,:,:,:] = (img[:,:,:]-128.0)/128.0
                except Exception as e:
                    print 'Error occured: %s, %s', str(e), fs[rnd]
                    

            #print "load image done"
            
            # train generator
            z = Variable(np.random.uniform(-1, 1, (batchsize, nz)).astype(np.dtype(np.float32)))
            x = gen(z)
            yl = dis(x)
            L_gen = F.softmax_cross_entropy(yl, Variable(np.zeros(batchsize).astype(np.dtype(np.int32))))
            L_dis = F.softmax_cross_entropy(yl, Variable(np.ones(batchsize).astype(np.dtype(np.int32))))
            ...
            x2 = Variable(x2)
            yl2 = dis(x2)
            L_dis += F.softmax_cross_entropy(yl2, Variable(np.zeros(batchsize).astype(np.dtype(np.int32))))
            ...
            if i%image_save_interval==0:
                ...
                vissize = 100
                z = zvis
                z[50:,:] = (np.random.uniform(-1, 1, (50, nz)).astype(np.dtype(np.float32)))
                z = Variable(z)
                x = gen(z, test=True)
                x = x.data
                for i_ in range(100):
